...
class Cell:
    ...
    def __init__(self, position_x, position_y, mine, name):
        ...
        if self.has_mine:
            ...
            try:
                cells["cell{0}".format(str(int(cells[name].x)) + str(int(cells[name].y) + 1))].number += 1  # right
                cells["cell{0}".format(str(int(cells[name].x)) + str(int(cells[name].y) - 1))].number += 1  # left
                cells["cell{0}".format(str(int(cells[name].x) + 1) + str(int(cells[name].y)))].number += 1  # bottom
                cells["cell{0}".format(str(int(cells[name].x) - 1) + str(int(cells[name].y)))].number += 1  # top

                cells["cell{0}".format(str(int(cells[name].x) - 1)) + str(int(cells[name].y) + 1)].number += 1  # top right
                cells["cell{0}".format(str(int(cells[name].x) - 1)) + str(int(cells[name].y) - 1)].number += 1  # top left

                cells["cell{0}".format(str(int(cells[name].x) + 1)) + str(int(cells[name].y) + 1)].number += 1  # bottom right
                cells["cell{0}".format(str(int(cells[name].x) + 1)) + str(int(cells[name].y) - 1)].number += 1  # bottom left
                print(self.number)
